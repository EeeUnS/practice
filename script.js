...
const settings = {
    ...
    hoverArea: 1, // part of the monster that activates hover
...
function HoverAction(action, alert) {
    return function() {
        cfg.hoverAction = action;
        if ( cfg.hoverAction && !interruptHover && (!alert || !interruptAlert) && monsters[target] && monsters[target].hasAttribute('onclick') ) {
            if ( shiftHeld && cfg.hoverShiftAction ) {
...
function ToggleHover() {
    ...
    if ( cfg.hoverAction && !interruptHover && !interruptAlert && monsters[target] && monsters[target].hasAttribute('onclick') ) {
        if ( shiftHeld && cfg.hoverShiftAction ) {
            cfg.hoverShiftAction(); }
        else if ( ctrlHeld && cfg.hoverCtrlAction ) {
            cfg.hoverCtrlAction(); }
...
function Settings() {
    ...
    if ( !document.getElementById('hvcfgstyle') ) {
        ...
        style.id = 'hvcfgstyle';
        style.innerHTML =
            '#mainpane { height: ' + (document.getElementById('navbar') ? '644' : '671') + 'px; overflow: auto; }' +
            '#mbcfg div { display: flex; margin: 2px; } #mbcfg input { margin: 0 2px 0 0; } #mbcfg input[type="text"] { width: 30px; }' +
    ...
    const Change = function(i,e) {
        return function() {
            ...
            switch ( settingsData[i][1][0] ) {
                ...
                case 't':
                case 's': prf[setting] = (value = input.value.replace(regexp.whitespace,'')) == '' ? above[setting] : value; break; }
    ...
    Save = function() { changed = false; localStorage.HVmbcfg = JSON.stringify(cfg); },
    Reset = function() {
    ...
    button.onclick = function() { if ( !changed || confirm('Save and exit?') ) { Save(); location.href = location.href; }};
    button = div.appendChild(document.createElement('button'));
    button.innerHTML = 'Exit';
    button.onclick = function() { if ( !changed || confirm('Exit without saving?') ) { location.href = location.href; }};
    button = div.appendChild(document.createElement('button'));
...
function handleKeyup(e) {
    ...
    ctrlHeld = e.ctrlKey;
    altHeld = e.altKey;
    cfg.hoverAction = defaultAction;
    if ( cfg.hoverAutoresume ) {
        interruptHover = false;
        if ( cfg.hoverAction && !interruptAlert && monsters[target] && monsters[target].hasAttribute('onclick') ) {
            if ( shiftHeld && cfg.hoverShiftAction ) {
                cfg.hoverShiftAction(); }
            else if ( ctrlHeld && cfg.hoverCtrlAction ) {
                cfg.hoverCtrlAction(); }
...
var cfg = {}, profile = JSON.parse(localStorage.HVmbp || '{ "p":0,"s1":0,"s2":0,"s3":0,"s4":0,"s5":0,"s6":0,"s7":0,"s8":0,"s9":0 }'),
    bindings = [], gem = [], target = false, cursor = -1, interruptHover, interruptAlert = false, shiftHeld = false,
    ctrlHeld = false, altHeld = false, defaultAction, bg, dummy = document.createElement('div'), mp = 1, sp = 1,
    monsters = [], monsterData = {}, log, timelog, combatlog, vitals, processing = false, csp = false;
...
function Enhance() {
    ...
    cursor = -1;
    timelog = JSON.parse(localStorage.HVtimelog || '{ "turn":0,"action":0,"round":1,"lastuse":{} }');
    combatlog = JSON.parse(localStorage.HVcombatlog || '{ "pdealt":{"hit":0,"crit":0,"miss":0},"mdealt":{"hit":0,"crit":0,"miss":0},' +
                                                         ...
                                                         '"mtaken":{"hit":0,"shit":0,"crit":0,"scrit":0,"miss":0},' +
                                                         '"used":{} }');
    if ( cfg.maxVitals ) {
    ...
    if ( cfg.mouseEngage ) {
        ...
        document.addEventListener('mouseup', function() { cfg.hoverAction = defaultAction; }, true); }
    ...
    if ( !document.getElementById('homosex') ) {
        ...
        style.innerHTML = '.btm6 { min-width: 195px; top: 0 !important; padding: 18px 3px 3px 1px !important; }' +
            ...
            '-566px; } #ckey_spirit { left: -421px; top: 45px;' : '-548px; } #ckey_spirit { left: -459px; top: 2px;') +
            ...
            'font-weight: bold; background: black; border-radius: 5px; }  .minfo > table { display: ruby; } .minfo td { padding: 1px; }' +
            '#minfo_0 { top: ' + ((cfg.showMonsterHP ? 61 : 55)+(cfg.vitalsAboveMonsters ? 65 : 0)) + 'px; }' +
            '#minfo_1 { top: ' + ((cfg.showMonsterHP ? 119 : 113)+(cfg.vitalsAboveMonsters ? 65 : 0)) + 'px; }' +
            '#minfo_2 { top: ' + ((cfg.showMonsterHP ? 177 : 171)+(cfg.vitalsAboveMonsters ? 65 : 0)) + 'px; }' +
            '#minfo_3 { top: ' + ((cfg.showMonsterHP ? 235 : 229)+(cfg.vitalsAboveMonsters ? 65 : 0)) + 'px; }' +
            '#minfo_4 { top: ' + ((cfg.showMonsterHP ? 293 : 287)+(cfg.vitalsAboveMonsters ? 65 : 0)) + 'px; }' +
            '#minfo_5 { top: ' + ((cfg.showMonsterHP ? 351 : 345)+(cfg.vitalsAboveMonsters ? 65 : 0)) + 'px; }' +
            '#minfo_6 { top: ' + ((cfg.showMonsterHP ? 409 : 403)+(cfg.vitalsAboveMonsters ? 65 : 0)) + 'px; }' +
            '#minfo_7 { top: ' + ((cfg.showMonsterHP ? 467 : 461)+(cfg.vitalsAboveMonsters ? 65 : 0)) + 'px; }' +
            '#minfo_8 { top: ' + ((cfg.showMonsterHP ? 525 : 519)+(cfg.vitalsAboveMonsters ? 65 : 0)) + 'px; }' +
            '#minfo_9 { top: ' + ((cfg.showMonsterHP ? 583 : 577)+(cfg.vitalsAboveMonsters ? 65 : 0)) + 'px; }' +
            ...
            '.mbar > div { cursor: pointer; } #barinfo { position: absolute; top: 650px; left: 1195px; cursor: pointer; }' +
    ...
    if ( document.querySelector('img[src$="finishbattle.png"]') ) {
        ...
        if ( cfg.monsterBar[0] ) {
            for ( var i = 0; i < monsters.length; i++ ) {
                document.getElementById('mbar_' + i).style.display = 'none'; }
            if ( cfg.monsterInfo ) {
        ...
        document.getElementById('homosex').innerHTML += '.minfo { display: none; }';
        var btcp;
            ...
            if ( cfg.monsterInfo || cfg.submitScans || cfg.monsterKeywords ) {
                ...
                xmlhttp.onreadystatechange = function() {
                    if ( xmlhttp.readyState == 4 && xmlhttp.status == 200 ) {
                        var response = JSON.parse(xmlhttp.responseText), content, index;
...
function MonsterInfo() {
    ...
    var info, onc, minfo, update, date = new Date(), year = date.getUTCFullYear(), month = date.getUTCMonth() + 1, day = date.getUTCDate();
    for ( var i = 0; i < monsters.length; i++ ) {
        if ( (info = monsterData.info[i]) ) {
            onc = monsters[i].hasAttribute('onclick');
            minfo = document.getElementById('minfo_' + i);
            update = info.lastUpdate.match(regexp.date);
            monsterData.recent[i] = !cfg.submitScans || (monsterData.name[i] == info.monsterName && (info.trainer === '' || info.plvl == 2250 ||
                                    (year - parseInt(update[1])) * 365 + (month - parseInt(update[2])) * 31 + day - parseInt(update[3]) < cfg.scanTime));
...
function Monsters() {
    ...
    for ( var i = 0; i < monsters.length; i++ ) {
        if ( (monster = monsters[i]) && monster.hasAttribute('onclick') ) {
            ...
            if ( cursor >= 0 ) Cursor();
            if ( target === i && cfg.hoverAction && !interruptHover && !interruptAlert ) {
                if ( shiftHeld && cfg.hoverShiftAction ) {
                    cfg.hoverShiftAction(); }
                else if ( ctrlHeld && cfg.hoverCtrlAction ) {
                    cfg.hoverCtrlAction(); }
                    ...
                    cfg.hoverAction(); }
                monster.click(); }
            if ( cfg.submitScans && !monsterData.recent[i] ) {
...
function SetTarget(i) { return function() {
    target = i;
    if ( cfg.hoverAction && !interruptHover && !interruptAlert && monsters[target].hasAttribute('onclick') ) {
        if ( shiftHeld && cfg.hoverShiftAction ) {
            cfg.hoverShiftAction(); }
        else if ( ctrlHeld && cfg.hoverCtrlAction ) {
            cfg.hoverCtrlAction(); }
...
function MouseEngage(e) {
    if ( e.which == 1 ) cfg.hoverAction = cfg.clickLeft;
    else if ( e.which == 2 ) cfg.hoverAction = cfg.clickMiddle;
    else if ( e.which == 3 ) cfg.hoverAction = cfg.clickRight; }
...
function ExtendQuickbar() {
    ...
    for ( var i = 0; i < cfg.quickbarExtend.length; i++ ) {
        ...
        switch ( cfg.quickbarExtend[i] ) {
            ...
            case 1: if ( gem[0] ) {
                        if ( cfg.raiseGem ) {
                            div.setAttribute('style', 'position: absolute; top: 50px; left: 622px'); }
...
function Disengage() { target = false; cfg.hoverAction = defaultAction; }

function BarEngage(i, actions) {
    return function() {
        if ( !monsters[i].hasAttribute('onclick') ) return;
        cfg.hoverAction = StrongestBar(actions);
        SetTarget(i)(); };}
...
function ProcessLog() {
    ...
    if ( cfg.showCooldowns || cfg.trackSpeed ) localStorage.HVtimelog = JSON.stringify(timelog);
    if ( cfg.trackDamage || cfg.trackUsage ) localStorage.HVcombatlog = JSON.stringify(combatlog); }
...
function TrackDrops() {
    ...
    var droplog = JSON.parse(localStorage.HVtrackdrops ||
    ...
    if ( cfg.trackDrops ) {
        ...
        while ( n-- > 0 ) {
            ...
            if ( (crystal = text.match(regexp.crystal)) ) {
                droplog.Crystal = (droplog.Crystal ? droplog.Crystal : 0) + parseInt(crystal[1]);
                if ( cfg.detailedCrystlog && (crystals = text.match(regexp.crystals)) ) {
                    droplog.Crystals[crystals[0]] = (droplog.Crystals[crystals[0]] ? droplog.Crystals[crystals[0]] : 0) + parseInt(crystal[1]); }}
...
function Profbar() {
    ...
    var profbar = document.getElementById('profbar'),
        droplog = JSON.parse(localStorage.HVtrackdrops || '{ "proficiency":{} }');
    if ( !profbar ) {
...
function ShowDrops(end) {
    ...
    var droplog = JSON.parse(localStorage.HVtrackdrops || '{}'), span, crystal, credit,
        peerless, legendary, magnificent, exquisite, superior, average, fair, crude, equipment,
...
function SettingsLink() {
    ...
    document.head.appendChild(document.createElement('style')).innerHTML = '#mbsettings { cursor: pointer; } #mbslink { padding: 8px 3px 5px; }' +
        '#mbsettings:hover #mbprofile { visibility: visible; } #mbprofile { visibility: hidden; position: absolute; left: ' + (def ? '227' : '179') +
        'px; top: 179px; width: max-content; height: max-content; padding: 5px; background: #EDEBDF; border: 1px solid #5C0D11; }';
    div.id = 'mbsettings';
    link.id = 'mbslink';
    link.onclick = Settings;
    if ( def ) {
        character.style.width = '228px';
        link.className = 'fl f4b';
        ...
        link.className = 'fc4 fal fcb';
        link.innerHTML = 'Monsterbation Settings'; }
    if ( regexp.profile.test(JSON.stringify(cfg.persona)) ) {
        ...
        const Set = function(i,p,s) {
            return function() {
                menu.querySelector('[style*="blue"').style.color = '';
                profile.p = p;
                if (p) {
                    profile['s' + p] = s; }
                localStorage.HVmbp = JSON.stringify(profile);
        ...
        item.innerHTML = cfg.name;
        item.onclick = Set(item, 0, 0);
        if ( profile.p == 0 ) {
            item.style.color = 'blue'; }
        for ( var i = 0; i < cfg.persona.length; i++ ) {
            if ( regexp.profile.test(JSON.stringify(cfg.persona[i])) ) {
                item = menu.appendChild(document.createElement('div'));
                item.innerHTML = '- ' + cfg.persona[i].name;
                item.onclick = Set(item, i+1, 0);
                if ( profile.p == i+1 && profile['s' + profile.p] == 0 ) {
                    item.style.color = 'blue'; }
                for ( var j = 0; j < cfg.persona[i].set.length; j++ ) {
                    if ( regexp.profile.test(JSON.stringify(cfg.persona[i].set[j])) ) {
                        item = menu.appendChild(document.createElement('div'));
                        item.innerHTML = '-- ' + cfg.persona[i].set[j].name;
                        item.onclick = Set(item, i+1, j+1);
